...
//        });
    ...
    //Enables observeID (id for a text box) for search searchElements (css selector) and inject matches 
    function enableSearchBox(observeID, searchElements, top) {
        //enable search box
        var myFilter = new ElementFilter(observeID, searchElements,{
            trigger:'keyup',
            cache:true,
...
//            if (transfers[attr.id]) { //ADRIAN transferscomment
...
//            }
    ...
    function selectGraphReal(graphType, data) {
        ...
        if (_.size(data) === 0) {
//            graph.graph && //graphcomment
//                graph.graph.clear();
            buttonContainer.hide();
            document.id('legend').hide();
            document.id('graphTitle').hide();
//            graph.slider.hide(); //graphcomment
            tableContainer.hide();
        ...
        document.id('graphTitle').show();


//        graph.slider.hide(); //graphcomment

//        graph.currentGraph = graphType; //graphcomment
        // Render the table
...
//            legend.fill(data, nodes); //legendcomment
        }

        // Hilight the current graph icon
        buttonContainer.getElements('.active').removeClass('active');
//        document.id(graph.currentGraph).addClass('active'); //graphcomment
        document.id(graphType).addClass('active'); // //graphcomment replacement
...
//        title.update(currentBreakdown, selectedNodes, selectedWithParents); //titlecomment
//        notes.update(currentBreakdown, selectedNodes, selectedWithParents); //notescomment
...
//        legend.expanded = {}; //legendcomment
    ...
    B.chooseGraphType = function (data) {
        return 'tableButton'; // //graphcomment addition
        ...
        if (negativeData) {
            ...
            if (positiveGraphs.contains(graphType)) {
                graphType = 'lineButton';
//                info.update(null, null, 'negativeData'); //infocomment
            }
    ...
    var hash = '';
    function updateHash() {
//        var newHash = graph.currentGraph + '/'; //graphcomment
        var newHash = 'tableButton' + '/'; // //graphcomment replacement

//        newHash += transfersCheckbox.hasClass('fullCheck') ? 1 : 0; //ADRIAN transferscomment
        newHash += '/';
    ...
    function processHash() {
        ...
        if (newHash !== hash) {
            ...
            if (transfers === '1') {
//                addTransfers(); //ADRIAN transferscomment
            } else {
//                removeTransfers(); //ADRIAN transferscomment
            }
...
//            graph.currentGraph = graphType; //graphcomment
    ...
    function resize(evt) {
        ...
        var titleDim     = document.id('graphTitle').getComputedSize();
        ...
        var legendsDim  = document.id('legend').getComputedSize();
        var winWidth = window.getSize().x;
        var width  = winWidth - (legendsDim.totalWidth + 75);
        var scrollSize = window.getScrollSize();
        width = width < minWidth ? minWidth : width;
        document.id('legend').setStyle('left', width + 20 + 'px');
...
//        if (graph.currentGraph === 'pieButton') { ////graphcomment
//           height -= 50;
//        }
...
//        if (graph.currentGraph === 'tableButton' && tableBody) { ////graphcomment
        if ('tableButton' === 'tableButton' && tableBody) { ////graphcomment replacement
        ...
        // Set title width
//        if (graph.currentGraph === 'tableButton') { ////graphcomment
        if ('tableButton' === 'tableButton') { //////graphcomment replacement
            visualize.setStyle('width', '100%');
            visualize.setStyle('margin', '0 auto');
            document.id('graphTitle').setStyle('margin', '0 0 0 0px');
//        } else if (graph.currentGraph === 'pieButton') { //graphcomment
//            visualize.setStyle('width', width);
//            visualize.setStyle('margin', '0 0 0 0');
//            document.id('graphTitle').setStyle('margin', '0 0 0 0px');
//        } else {
//            visualize.setStyle('margin', '0 0 0 0px');
//            //document.id('graphTitle').setStyle('margin', '0 0 0 80px');
//            visualize.setStyle('width', width);
        }
...
//        if (width && height)
//            graph.size([width, height]); ////graphcomment
    }
...
//        msd.dropdowns.Ledger.element.addEvent('click:relay(.transfersCheckbox, .transfersLabel)', function () { //ADRIAN stop graph
//            if (transfersChecked) {
...
//        buttonContainer.addEvent('click:relay(.graphButton.enabled)', function () { //ADRIAN stop graph
////            graph.currentGraph = this.get('id'); //graphcomment
...
//        graph.container.addEvent('mouseleave', mouseOutOnEverything); //graphcomment //legendcomment

//        graph.container.addEvent('mouseout', mouseOutOnEverything); //graphcomment //legendcomment
        //if (!Browser.firefox) {
...
//        window.onhashchange = processHash; //adrian hashcomment
    ...
    function renderSVG() {
        ...
        var titleText = document.id('graphTitle').get('text');
...
//        downloadSVG( //legendcomment
//            document.id('graph').get('html'),
//            document.id('graphTitle').get('html'),
//            JSON.stringify(legend.state));
    ...
    B.checkEmpty = function (quiet) {
        ...
        if (reset && !quiet) {
//            info.update(null, null, 'reset'); //infocomment
        }
...
//                      info.update(null, null, 'toexp'); //infocomment
                      special.expenseBreakdown.fireEvent('checkClick');
                }
                if (special.Revenues.anyCheck() && !special.Expenses.anyCheck()) {
//                      info.update(null, null, 'torev'); //infocomment
